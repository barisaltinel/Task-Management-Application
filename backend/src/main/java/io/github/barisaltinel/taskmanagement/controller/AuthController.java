...
public class AuthController {
  ...
  public ResponseEntity<ApiDtos.UserResponse> registerUser(
      @Valid @RequestBody ApiDtos.AuthRegisterRequest request) {
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(ApiMapper.toUserResponse(userService.register(ApiMapper.toUser(request))));
  }

  @PostMapping("/login")
  public ResponseEntity<ApiDtos.AuthResponse> login(
      @Valid @RequestBody ApiDtos.AuthLoginRequest request) {
    AuthService.AuthSession session = authService.login(request.email(), request.password());
    ApiDtos.AuthResponse response =
        new ApiDtos.AuthResponse(
