...
public class TaskController {
  ...
  public ResponseEntity<List<ApiDtos.TaskResponse>> getAllTasks() {
    return ResponseEntity.ok(
        taskService.getAllTasks().stream().map(ApiMapper::toTaskResponse).toList());
  }
  ...
  public ResponseEntity<ApiDtos.TaskResponse> createTask(
      @Valid @RequestBody ApiDtos.TaskUpsertRequest request) {
    Task createdTask =
        taskService.create(ApiMapper.toTask(request), request.projectId(), request.assigneeId());
  ...
  public ResponseEntity<ApiDtos.TaskResponse> updateTask(
    ...
    Task updatedTask =
        taskService.update(
            id, ApiMapper.toTask(request), request.projectId(), request.assigneeId());
