...
public class CommentController {
    ...
    public ResponseEntity<List<ApiDtos.CommentResponse>> getAllComments() {
        return ResponseEntity.ok(commentService.getAllComments().stream().map(ApiMapper::toCommentResponse).toList());
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('TEAM_MEMBER', 'TEAM_LEADER', 'PROJECT_MANAGER', 'ADMIN')")
    public ResponseEntity<ApiDtos.CommentResponse> getCommentById(@PathVariable Long id) throws CommentNotFoundException {
        return ResponseEntity.ok(ApiMapper.toCommentResponse(commentService.findById(id)));
    }

    @PostMapping
    @PreAuthorize("hasAnyRole('TEAM_MEMBER', 'TEAM_LEADER', 'PROJECT_MANAGER', 'ADMIN')")
    public ResponseEntity<ApiDtos.CommentResponse> createComment(@Valid @RequestBody ApiDtos.CommentCreateRequest request) {
